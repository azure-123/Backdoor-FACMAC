...
class Particle(MultiAgentEnv):
    ...
    def get_obs(self, backdoor=None):
        """ Returns all agent observations in a list """
        obs_n = []
        for i, _ in enumerate(self.world.policy_agents):
            obs = self.get_obs_agent(i, backdoor)
            obs_n.append(obs)
        return obs_n

    def get_obs_agent(self, agent_id, backdoor=None):
        """ Returns observation for agent_id """
        obs = self.env._get_obs(self.world.policy_agents[agent_id], backdoor)
    ...
    def get_state(self, backdoor=None, team=None):
        state = np.concatenate(self.get_obs(backdoor))
        return state
