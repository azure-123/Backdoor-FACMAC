...
class Scenario(BaseScenario):
    ...
    def prey_policy(self, agent, world):
        ...
        # for _agent in world.agents:
        #     if _agent.name != agent.name:
        #         delta_pos = _agent.state.p_pos - np.array([0.5, 0.5])
        #         dist = np.sqrt(np.sum(np.square(delta_pos)))
        #         dist_min = _agent.size + agent.size
        #         if dist >= dist_min:
        #             chosen_action = np.array(agent.state.p_pos - np.array([0.5, 0.5]), dtype=np.float32) # np.array([x[best_idx], y[best_idx]], dtype=np.float32)
        #         else:
    ...
    def observation(self, agent, world, backdoor):
        ...
        if backdoor is None:
            ...
            for other in world.agents:
                ...
                dist = np.sqrt(np.sum(np.square(other.state.p_pos - agent.state.p_pos))) # 其他智能体与中心智能体之间的距离
                if agent.view_radius >= 0 and dist <= agent.view_radius:
                    comm.append(other.state.c) # 涉及到通讯，计入通讯状态
                    other_pos.append(other.state.p_pos - agent.state.p_pos)
                    if not other.adversary: # 其他智能体不是对手智能体
                        other_vel.append(other.state.p_vel) # 将其他智能体的速度计入
                else:
                    other_pos.append(np.array([0., 0.])) # 在中心智能体的视野范围之外，没有信息
                    if not other.adversary:
                        other_vel.append(np.array([0., 0.]))
            return np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + entity_pos + other_pos + other_vel) # 中心智能体的速度+中心智能体的位置+路标的位置+其他智能体的位置+队友智能体的速度
        else:
            if backdoor == agent.name:
                agent_pos = np.array([0.5, 0.5])
            else:
                agent_pos = agent.state.p_pos
            entity_pos = []
            for entity in world.landmarks:
                dist = np.sqrt(np.sum(np.square(entity.state.p_pos - agent_pos)))
                if not entity.boundary and (agent.view_radius >= 0) and dist <= agent.view_radius:
                    entity_pos.append(entity.state.p_pos - agent_pos)
                else:
                    entity_pos.append(np.array([0., 0.]))
            # communication of all other agents
            comm = []
            ...
            for other in world.agents:
                if other is agent: continue
                if backdoor == other.name:
                    other_backdoor_pos = np.array([0.5, 0.5])
                else:
                    other_backdoor_pos = other.state.p_pos
                dist = np.sqrt(np.sum(np.square(other_backdoor_pos - agent_pos))) # 其他智能体与中心智能体之间的距离
                if agent.view_radius >= 0 and dist <= agent.view_radius:
                    comm.append(other.state.c) # 涉及到通讯，计入通讯状态
                    other_pos.append(other_backdoor_pos - agent_pos)
                    if not other.adversary: # 其他智能体不是对手智能体
                        other_vel.append(other.state.p_vel) # 将其他智能体的速度计入
                else:
                    other_pos.append(np.array([0., 0.])) # 在中心智能体的视野范围之外，没有信息
                    if not other.adversary:
                        other_vel.append(np.array([0., 0.]))        
            return np.concatenate([agent.state.p_vel] + [agent_pos] + entity_pos + other_pos + other_vel) # 中心智能体的速度+中心智能体的位置+路标的位置+其他智能体的位置+队友智能体的速度
