...
def evaluate_sequential(args, runner):
    ...
    for _ in range(args.test_nepisode):
        episode_return = runner.run(test_mode=True)
        test_return_list.append(episode_return)
        print(_,"-----------------",episode_return)
    test_mean = np.mean(np.array(test_return_list))
    test_std = np.std(np.array(test_return_list))
    zero_count = test_return_list.count(0)
    zero_rate = zero_count / len(test_return_list)
    print("mean: ", test_mean)
    print("std: ", test_std)
    print("zero rate: ", zero_rate)
    if args.save_replay:
        runner.save_replay()
...
def run_sequential(args, logger):
    ...
    runner.setup(scheme=scheme, groups=groups, preprocess=preprocess, mac=mac, adv_mac=adv_mac)

    # Learner
    learner = le_REGISTRY[args.learner](mac, buffer.scheme, logger, args)
    if args.train_adversary:
        adv_learner = le_REGISTRY[args.learner](mac, adv_buffer.scheme, logger, args)
    else:
        adv_learner = None
    ...
    while runner.t_env <= args.t_max:
        if args.train_adversary is False:
        # Run for a whole episode at a time
            ...
            if getattr(args, "runner_scope", "episodic") == "episodic":
                adv_episode_batch, episode_batch = runner.run(test_mode=False, learner=learner)
                buffer.insert_episode_batch(episode_batch)
                ...
                if adv_buffer.can_sample(args.batch_size) and (adv_buffer.episodes_in_buffer > getattr(args, "buffer_warmup", 0)):
                    ...
                    adv_learner.train(episode_sample, runner.t_env, episode)
            elif getattr(args, "runner_scope", "episode") == "transition":
                runner.run(test_mode=False,
                        buffer=buffer,
                        learner=learner,
        ...
        if args.save_model and (runner.t_env - model_save_time >= args.save_model_interval or model_save_time == 0):
            ...
            if args.train_adversary:
                model_save_time = runner.t_env
                adv_save_path = os.path.join(args.local_adversary_path, "models", args.unique_token, str(runner.t_env))
                #"results/models/{}".format(unique_token)
                os.makedirs(adv_save_path, exist_ok=True)
                logger.console_logger.info("Saving adversary models to {}".format(adv_save_path))
